...
__global__ void build_particle_cell_buckets(uint32_t particle_count, ParticleArray parray, ParticleBuffer pbuffer, Partition partition) {
	uint32_t parid = blockIdx.x * blockDim.x + threadIdx.x;
	if(parid >= particle_count) {
		return;
	}
	...
	auto pidic																							 = atomicAdd(pbuffer.ppcs + blockno * config::G_BLOCKVOLUME + cellno, 1);
	pbuffer.cellbuckets[blockno * config::G_PARTICLE_NUM_PER_BLOCK + cellno * config::G_MAX_PPC + pidic] = static_cast<int>(parid);//NOTE:Explicit narrowing conversation.
}
...
__global__ void rasterize(uint32_t particle_count, const ParticleArray parray, Grid grid, const Partition partition, float dt, float mass, vec3 v0) {
	uint32_t parid = blockIdx.x * blockDim.x + threadIdx.x;
	if(parid >= particle_count) {
		return;
	}
...
__global__ void array_to_buffer(ParticleArray parray, ParticleBuffer<MaterialE::J_FLUID> pbuffer) {
	uint32_t blockno = blockIdx.x;
	int pcnt		 = pbuffer.ppbs[blockno];
	auto* bucket	 = pbuffer.blockbuckets + static_cast<size_t>(blockno) * config::G_PARTICLE_NUM_PER_BLOCK;
	for(int pidib = static_cast<int>(threadIdx.x); pidib < pcnt; pidib += static_cast<int>(blockDim.x)) {
		auto parid = bucket[pidib];
		auto pbin  = pbuffer.ch(_0, pbuffer.binsts[blockno] + pidib / config::G_BIN_CAPACITY);
		/// pos
...
template<typename ParticleArray>
__global__ void array_to_buffer(ParticleArray parray, ParticleBuffer<MaterialE::FIXED_COROTATED> pbuffer) {
	uint32_t blockno = blockIdx.x;
	int pcnt		 = pbuffer.ppbs[blockno];
	auto* bucket	 = pbuffer.blockbuckets + static_cast<size_t>(blockno) * config::G_PARTICLE_NUM_PER_BLOCK;
	for(int pidib = static_cast<int>(threadIdx.x); pidib < pcnt; pidib += static_cast<int>(blockDim.x)) {
...
template<typename ParticleArray>
__global__ void array_to_buffer(ParticleArray parray, ParticleBuffer<MaterialE::SAND> pbuffer) {
	uint32_t blockno = blockIdx.x;
	int pcnt		 = pbuffer.ppbs[blockno];
	auto* bucket	 = pbuffer.blockbuckets + static_cast<size_t>(blockno) * config::G_PARTICLE_NUM_PER_BLOCK;
	for(int pidib = static_cast<int>(threadIdx.x); pidib < pcnt; pidib += static_cast<int>(blockDim.x)) {
...
template<typename ParticleArray>
__global__ void array_to_buffer(ParticleArray parray, ParticleBuffer<MaterialE::NACC> pbuffer) {
	uint32_t blockno = blockIdx.x;
	int pcnt		 = pbuffer.ppbs[blockno];
	auto* bucket	 = pbuffer.blockbuckets + static_cast<size_t>(blockno) * config::G_PARTICLE_NUM_PER_BLOCK;
	for(int pidib = static_cast<int>(threadIdx.x); pidib < pcnt; pidib += static_cast<int>(blockDim.x)) {
...
__global__ void update_grid_velocity_query_max(uint32_t block_count, Grid grid, Partition partition, float dt, float* max_vel) {
	...
	/// 32];
	extern __shared__ float sh_maxvels[];//NOLINT(modernize-avoid-c-arrays) Cannot declare runtime size shared memory as std::array
...
__global__ void g2p2g(float dt, float new_dt, const ParticleBuffer<MaterialE::J_FLUID> pbuffer, ParticleBuffer<MaterialE::J_FLUID> next_pbuffer, const Partition prev_partition, Partition partition, const Grid grid, Grid next_grid) {
	...
	for(int pidib = static_cast<int>(threadIdx.x); pidib < ppb; pidib += static_cast<int>(blockDim.x)) {
		int source_blockno;
		int source_pidib;
		ivec3 base_index;
		{
			int advect = next_pbuffer.blockbuckets[src_blockno * config::G_PARTICLE_NUM_PER_BLOCK + pidib];
			...
			source_blockno = pbuffer.binsts[source_blockno] + source_pidib / config::G_BIN_CAPACITY;
		}
		vec3 pos;
		float J;
		{
...
#pragma unroll 3
		...
		vec9 contrib;
		{
			...
			{
				auto particle_bin									 = next_pbuffer.ch(_0, next_pbuffer.binsts[src_blockno] + pidib / config::G_BIN_CAPACITY);
		...
		local_base_index = (pos * config::G_DX_INV + 0.5f).cast<int>() - 1;
		{
			...
			next_pbuffer.add_advection(partition, local_base_index - 1, dirtag, pidib);
...
template<typename Partition, typename Grid>
__global__ void g2p2g(float dt, float new_dt, const ParticleBuffer<MaterialE::FIXED_COROTATED> pbuffer, ParticleBuffer<MaterialE::FIXED_COROTATED> next_pbuffer, const Partition prev_partition, Partition partition, const Grid grid, Grid next_grid) {
	static constexpr uint64_t NUM_VI_PER_BLOCK = static_cast<uint64_t>(config::G_BLOCKVOLUME) * 3;
	static constexpr uint64_t NUM_VI_IN_ARENA  = NUM_VI_PER_BLOCK << 3;
...
template<typename Partition, typename Grid>
__global__ void g2p2g(float dt, float new_dt, const ParticleBuffer<MaterialE::SAND> pbuffer, ParticleBuffer<MaterialE::SAND> next_pbuffer, const Partition prev_partition, Partition partition, const Grid grid, Grid next_grid) {
	static constexpr uint64_t NUM_VI_PER_BLOCK = static_cast<uint64_t>(config::G_BLOCKVOLUME) * 3;
	static constexpr uint64_t NUM_VI_IN_ARENA  = NUM_VI_PER_BLOCK << 3;
	...
	for(int pidib = static_cast<int>(threadIdx.x); pidib < next_pbuffer.ppbs[src_blockno]; pidib += static_cast<int>(blockDim.x)) {
		int source_blockno;
		int source_pidib;
		ivec3 base_index;
		{
...
template<typename Partition, typename Grid>
__global__ void g2p2g(float dt, float new_dt, const ParticleBuffer<MaterialE::NACC> pbuffer, ParticleBuffer<MaterialE::NACC> next_pbuffer, const Partition prev_partition, Partition partition, const Grid grid, Grid next_grid) {
	static constexpr uint64_t NUM_VI_PER_BLOCK = static_cast<uint64_t>(config::G_BLOCKVOLUME) * 3;
	static constexpr uint64_t NUM_VI_IN_ARENA  = NUM_VI_PER_BLOCK << 3;
...
__global__ void update_partition(uint32_t block_count, const int* __restrict__ source_nos, const Partition partition, Partition next_partition) {
	...
	uint32_t sourceNo					= source_nos[blockno];
	auto sourceBlockid					= partition.active_keys[sourceNo];
...
__global__ void update_buckets(uint32_t block_count, const int* __restrict__ source_nos, const ParticleBuffer pbuffer, ParticleBuffer next_pbuffer) {
	...
	if(threadIdx.x == 0) {
		...
		next_pbuffer.ppbs[blockno] = pbuffer.ppbs[sourceNo[0]];
	}
	__syncthreads();

	auto pcnt = next_pbuffer.ppbs[blockno];
	for(int pidib = static_cast<int>(threadIdx.x); pidib < pcnt; pidib += static_cast<int>(blockDim.x)) {
		next_pbuffer.blockbuckets[blockno * config::G_PARTICLE_NUM_PER_BLOCK + pidib] = pbuffer.blockbuckets[sourceNo[0] * config::G_PARTICLE_NUM_PER_BLOCK + pidib];
	}
}
...
__global__ void retrieve_particle_buffer(Partition partition, Partition prev_partition, ParticleBuffer pbuffer, ParticleBuffer next_pbuffer, ParticleArray parray, int* parcnt) {
	int pcnt			  = next_pbuffer.ppbs[blockIdx.x];
	ivec3 blockid		  = partition.active_keys[blockIdx.x];
	auto advection_bucket = next_pbuffer.blockbuckets + blockIdx.x * config::G_PARTICLE_NUM_PER_BLOCK;
	// auto particle_offset = pbuffer.binsts[blockIdx.x];
	for(int pidib = static_cast<int>(threadIdx.x); pidib < pcnt; pidib += static_cast<int>(blockDim.x)) {
		...
		auto source_bin		= pbuffer.ch(_0, pbuffer.binsts[source_blockno] + source_pidib / config::G_BIN_CAPACITY);
