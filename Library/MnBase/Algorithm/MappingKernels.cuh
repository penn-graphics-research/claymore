...
__global__ void mark_boundary(int num, const EntryType *_entries, MarkerType *_marks, BinaryOp op)
{
...
__global__ void exclusive_scan_inverse(CounterType num, const IndexType *_map, IndexType *_mapInv)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= num)
		return;
	auto mapIdx = _map[idx];
	if (mapIdx != _map[idx + 1])
		_mapInv[mapIdx] = idx;
}
...
__global__ void set_range_inverse(int count, const MappingType *_toPackedRangeMap, const MappingType *_toRangeMap, CounterType *_numPackedRange, MappingType *_rangeIds, MappingType *_rangeLeftBound, MappingType *_rangeRightBound)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= count)
		return;
	...
	if (idx == count - 1)
		*_numPackedRange = packedRangeIdx + 1;
}
