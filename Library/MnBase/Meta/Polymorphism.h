#ifndef __POLYMORPHISM_H_
#define __POLYMORPHISM_H_
...
namespace mn {
	...
	template<typename... TFs> overload(TFs...)->overload_set<TFs...>;
#else
...
struct overload_set : TF, overload_set<TFs...> {
    ...
    overload_set(TFFwd&& f, TFsFwd&&... fs)
        : TF{ std::forward<TFFwd>(f) }, overload_set<TFs...>{ std::forward<TFsFwd>(fs)... }
    {
    }
};
