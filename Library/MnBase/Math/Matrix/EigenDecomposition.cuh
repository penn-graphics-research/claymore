...
template <typename T>
__forceinline__ __device__ void fastEigenvalues(const T *mat, T *lambda) {
  ...
  double p = ((double)1 / 6) * (a00 * a00 + a11 * a11 + a22 * a22 +
                                2 * (a12_sqr + a13_sqr + a23_sqr));
  double q = (double).5 *
                 (a00 * (a11 * a22 - a23_sqr) - a11 * a13_sqr - a22 * a12_sqr) +
             mat[3] * mat[6] * mat[7];
...
fastEigenvectors(const T *mat, const T *lambda,
  ...
  for (int i = 0; i < 3; ++i)
    tmp[i] *= dtmp[3];
  double v1_orthogonal[3] = {tmp[0], tmp[1],
                             tmp[2]}; // 6m+2a+1d+1s (tweak: 5m+1a+1d+1s)
  // Eigen::Matrix<T, 3, 2> other_v;
  double other_v[6]; ///< 3x2
  other_v[0] = v1_orthogonal[0], other_v[1] = v1_orthogonal[1],
  ...
  double A_reduced[4] = {
      tmp[0] * other_v[0] + tmp[2] * other_v[1] + tmp[4] * other_v[2],
  ...
  if (sgn(norm2) > 0) {
    ...
    v3[0] = (other_v[0] * C3[j * 2 + 0] + other_v[3] * C3[j * 2 + 1]) *
            one_over_sqrt;
    v3[1] = (other_v[1] * C3[j * 2 + 0] + other_v[4] * C3[j * 2 + 1]) *
            one_over_sqrt;
    v3[2] = (other_v[2] * C3[j * 2 + 0] + other_v[5] * C3[j * 2 + 1]) *
            one_over_sqrt;
  } else {
    v3[0] = other_v[0];
    v3[1] = other_v[1];
...
template <typename T>
__forceinline__ __device__ void makePD3d(T *mat, T eps = 1e-6) {
...
template <typename T>
__forceinline__ __device__ void makePD2d(T *mat, T eps = 1e-6) {
  // based on
