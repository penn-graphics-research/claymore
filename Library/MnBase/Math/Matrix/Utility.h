...
template <typename T> constexpr T abs(const T n) noexcept {
  return n > 0 ? n : -n;
}

template <typename T>
constexpr void orthogonalVector(const T *input, T *output) noexcept {
...
template <typename T>
constexpr void cross(const T *operand1, const T *operand2, T *output) {
