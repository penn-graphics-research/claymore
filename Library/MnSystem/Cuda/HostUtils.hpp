...
#define checkThrustErrors(func)                                                \
  try {                                                                        \
    ...
    std::cout << std::string(__FILE__) << ":" << __LINE__ << " " << e.what()   \
              << std::endl;                                                    \
  }
...
template <class T>
__inline__ __host__ thrust::device_ptr<T>
getDevicePtr(thrust::device_vector<T> &V) {
...
inline void reportMemory(std::string msg) {
  ...
  printf("GPU memory usage (%s): used = %f, free = %f MB, total = %f MB\n",
         msg.data(), used_db / 1024.0 / 1024.0, free_db / 1024.0 / 1024.0,
